...
export function buildDesignReviewPrompt(input: BuildDesignReviewPromptInput): string {
  ...
  return [
    ...
    '- Apply the source role model: main: and symbol: drive non-Usage completeness (subject to programmatic honesty); dep: checks main implementation accuracy and does not block COMPLETE when unchecked; use: is Usage/integration only — mark [x] only when Usage documents the pattern with illustrative (non-verbatim) examples; test: verifies behavioral alignment in Component Design / Data Flow without requiring test file or class names in the design and does not block when unchecked.',
